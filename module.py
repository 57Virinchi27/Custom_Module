...
#painting an image
def paint_image(color,color_range):
...
#Draws a rectangle
def draw_rectangle(color, outline = 2):
...
#Edge Cascade
def img_EDGECASCADE_CANNY(image_path):
...
#Contours
def img_FIND_CONTOURS(image_path):
...
def split_COLORSPACES(image_path):
